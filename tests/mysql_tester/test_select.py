...
class TestGetAndFirst:
    ...
    def test_get_with_take(self, user_model):
        ...
        for i in range(5):
            user_model.create(
                {'name': f'TakeTest{i}', 'email': f'take{i}@test.com', 'age': 20 + i, 'status': 1, 'score': 80.0 + i})
...
class TestTakeAndOffset:
    ...
    def test_offset_invalid_number(self, user_model):
        """测试 offset() 无效数字"""
        # offset(0) 是有效的，只有负数应该抛异常
        sql = user_model.offset(0).tosql()
        assert 'offset 0' in sql.lower()
